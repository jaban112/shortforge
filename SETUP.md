...
## 0. 뚝딱 버전 (키·클라우드·API 전부 없음) — `shortforge web`

```bash
unzip shortforge_v2.0.0.zip && cd shortforge && bash install.sh
```
끝. 브라우저에 http://127.0.0.1:8787 이 뜬다. 거기서:
1. **YouTube 로그인** → 크로미움 창이 뜬다 → 평소처럼 로그인 (한 번만; 프로필에 저장됨)
2. **Instagram 로그인** → 같은 방식 (선택)
3. **확인** → 점이 초록이면 됨
4. **지금 1편 만들어 올리기** → 5분 뒤 아래 기록에 링크
5. **자동화 스위치 ON** → 매일 09:00·21:00(바꿀 수 있음)에 알아서. 크롬북 Linux가 떠 있으면 돈다(서비스가 자동 시작).

원리: 이 앱은 **네 브라우저를 대신 조작**한다(YouTube Studio·instagram.com 화면을 사람처럼 클릭). API 키·Google Cloud·Meta 앱이 필요 없는 유일한 방식이다. 대가: 사이트 UI가 바뀌면 깨질 수 있다 → 실패하면 앱이 **스크린샷을 찍어 화면에 띄우니** 그걸 보내라, 셀렉터를 고친다. 로그인 세션은 몇 주~몇 달 유지되고, 풀리면 점이 빨개지니 다시 로그인만 하면 된다.

아래 1~7은 API 방식(GitHub Actions 무인·키 필요)이다. 뚝딱 버전이면 안 읽어도 된다.

---

# (API 방식) 30분 안에 무인 가동까지
